...
import { BookingType, BookingStatus, RecurrenceFrequency } from '../entities/BookingJob';
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    if (bookingType === BookingType.ONE_TIME) {
    ...
    } else if (bookingType === BookingType.RECURRING) {
    ...
    logger.info(`Created booking job ${bookingJob.id} for ${userEmail} - ${amenityName}`);
...
router.get('/user/:email', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to fetch booking jobs:', error);
    res.status(500).json({
      success: false,
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (targetTime !== undefined) updates.targetTime = targetTime;
    ...
    if (preferredDaysOfWeek !== undefined) updates.preferredDaysOfWeek = preferredDaysOfWeek;
    ...
    logger.info(`Updated booking job ${id}`);

  } catch (error) {
    logger.error('Failed to update booking job:', error);
    res.status(500).json({
      success: false,
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    ...
    logger.info(`Deleted booking job ${id}`);

  } catch (error) {
    logger.error('Failed to delete booking job:', error);
    res.status(500).json({
      success: false,
...
router.post('/:id/pause', async (req: Request, res: Response) => {
  try {
    ...
    logger.info(`Paused booking job ${id}`);

  } catch (error) {
    logger.error('Failed to pause booking job:', error);
    res.status(500).json({
      success: false,
...
router.post('/:id/resume', async (req: Request, res: Response) => {
  try {
    const job = await bookingService.updateBookingJob(req.params.id, { status: BookingStatus.ACTIVE, isActive: true });
    if (!job) {
...
router.delete('/slots/:slotId', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to delete booked slot:', error);
